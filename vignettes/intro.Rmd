...
has_nld <- "nld" %in% cpp11tesseract::tesseract_info()$available
...
eng <- tesseract("eng")
text <- cpp11tesseract::ocr("http://jeroen.github.io/images/testocr.png", engine = eng)
cat(text)
```
...
```{r}
results <- cpp11tesseract::ocr_data("http://jeroen.github.io/images/testocr.png", engine = eng)
results
```
...
text <- cpp11tesseract::ocr(pngfile)
cat(text)
```
...
```{r}
cpp11tesseract::tesseract_info()['version']
```
