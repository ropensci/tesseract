...
has_chi_sim <- "chi_sim" %in% tesseract::tesseract_info()$available
...
![test](../man/figures/testocr.png)

```{r}
library(tesseract)
file <- system.file("examples", "testocr.png", package = "tesseract")
eng <- tesseract("eng")
text <- tesseract::ocr(file, engine = eng)
cat(text)
```
...
```{r}
results <- tesseract::ocr_data(file, engine = eng)
results
```
...
![chinese](../man/figures/chinese.png)
...
```{r}
text2 <- readLines(
  system.file("examples", "chinese.txt", package = "tesseract")
)
...
![receipt](../man/figures/receipt.png)
...
# relative paths will not work (i.e., use /home/downloads instead of ~/downloads)
dir <- tempdir()
tesseract_download("chi_sim_vert", datapath = dir, best = TRUE)
tesseract_download("chi_sim", datapath = dir, best = TRUE)
...
```{r, eval = FALSE}
file <- system.file("examples", "polytonicgreek.png", package = "tesseract")

# download the best models
dir <- tempdir()
tesseract_download("grc", datapath = dir, best = TRUE)
tesseract_contributed_download("grc_hist", datapath = dir, best = TRUE)
...
Here is the first element of the list retured by Textract:
...
```{r}
file <- system.file("examples", "tealbook.png", package = "tesseract")
text <- ocr(file)
...
for (i in seq_along(text)) {
  ...
  names(res) <- c("variable", "y1964", "y1965est", "y1965q1",
